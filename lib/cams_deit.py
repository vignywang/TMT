...
def apply_colormap(cam, cmap='viridis'):
    heatmap = plt.get_cmap(cmap)(cam)[..., :3] * 255
    heatmap = heatmap.astype(np.uint8)
    return heatmap
def blend_cam(image, cam, es_box=[0,0,1,1], blend=False, ratio=(0.6, 0.4)):
    if blend:
        I = np.zeros_like(cam)
        x1, y1, x2, y2 = es_box
        I[y1:y2, x1:x2] = 1
    ...
    heatmap = apply_colormap(cam, cmap='turbo_r')
