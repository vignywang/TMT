...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., vis=None):
        super().__init__()
    ...
    def forward(self, x, policy, flag=False):
        B, N, C = x.shape
        ...
        if policy is None:
            ...
            if flag:
                ...
                weights_r = [attn, attn_policy]
            else:
                attn, _ = self.softmax_with_policy(attn, policy)
...
class Block(nn.Module):
    ...
    def forward(self, x, policy=None, flag=False):

        o, weights = self.attn(self.norm1(x), policy, flag)
